...
def calc_move_info(board, piece, x, r, total_holes_bef, total_blocking_bloks_bef, heights_bef):
    """Calculate information based on the current play"""
    heights_sum_bef = sum(heights_bef)
    total_wells_bef = sum(ga.calc_wells(heights_bef))
    max_height_bef = max(heights_bef)
    piece['rotation'] = r
    piece['y']        = 0
    piece['x']        = x
    ...
    for x2 in range(0, BOARDWIDTH):
        ...
        # max_height += b[2]
    heights = ga.calc_heights(new_board)
    heights_sum = sum(heights)
    total_wells = ga.calc_wells(heights)
    max_height = max(heights)

    new_holes = total_holes
    new_blocking_blocks = total_blocking_block
    new_heights_sum = heights_sum
    new_wells = sum(total_wells)
    new_max_height = max_height
    new_num_wells = ga.calc_num_wells(total_wells)
    return [True, num_removed_lines, new_holes, new_blocking_blocks, new_num_wells, new_wells, new_max_height, piece_sides, floor_sides, wall_sides]
...
def calc_initial_move_info(board):
    ...
    heights = ga.calc_heights(board)
    # heights_sum = sum(heights)
    # total_wells = sum(ga.calc_wells(heights))
    # max_height = max(heights)

    return total_holes, total_blocking_bocks, heights
